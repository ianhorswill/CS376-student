...
public class BarGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Text.text = name;
        // Text.text = BarGraph.name; ---- alternative code
        // Text.text = this.name; ---- alternative code
        // each GameObject has a name field (more or less)
        ...
        //
        if (this.Min < 0) // checking if this should be a signedDisplay
        {
            signedDisplay = true;
            ...
            BarTransform.localPosition = new Vector3(pos.x, pos.y, pos.z); 
        } 
    }
    /// <summary>
    ...
    public void SetReading(float value)
    {
        ...
        // outside the widget.
        if (value < this.Min) //  when below range
        {

            color = Color.red;
            value = this.Min;
        }
        else if (value > this.Max) // when above range
        {
            color = Color.red;
            value = this.Max;
        }
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        // if signedDisplay, cut the scale in half
        if (signedDisplay)
        {
            BarTransform.localScale = new Vector3(value/2,
            ...
            BarTransform.localScale = new Vector3 (value, 
                                                    BarTransform.localScale.y, 
                                                    BarTransform.localScale.z);
        } 
    }
    ...
    public static BarGraph Find(string name, Vector2 position, float min, float max) {
        ...
        BarGraphTable.Add(name, bgComponent);
